...
def problem_one(request):
    ...
    students_gpa = Student.objects.order_by('-gpa').filter(gpa__gte=3.5)
    student_name = Student.objects.order_by('first_name', 'last_name')
    for student_info in student_name and students_gpa:
      
      print(f'Full Name: {student_info.first_name} {student_info.last_name} GPA: {student_info.gpa}')
...
def problem_two(request):
  ...
  instructors = Instructor.objects.order_by('-hire_date__year').filter(hire_date__year__lte=2010)
  
  

  for instructors_name in instructors:
    
    print(f'Full Name: {instructors_name.first_name} {instructors_name.last_name}')
    print(f'Hire Date: {instructors_name.hire_date}')
...
def problem_three(request):
  ...
  instructor_info = Instructor.objects.filter(id=2)
  instructor_courses = Course.objects.order_by('name').filter(instructor_id=2)

  for instructor_detail in instructor_info:
    print(f'Instructor Name: {instructor_detail.first_name} {instructor_detail.last_name}')
    print(f'Courses:')
    break
  for instructor_detail in instructor_courses:
    print(f'-{instructor_detail.name}')
...
def problem_five(request):
  ...
  p = Student.objects.create(id=11,first_name="Kyle", last_name="Harwood", year=2022, gpa=3.8)
  p.save()
 
  check_creation = Student.objects.filter(id=11)
  for student_check in check_creation:
    print(f'Id: {student_check.id}')
    print(f'Full Name: {student_check.first_name} {student_check.last_name}')
    print(f'Year: {student_check.year}')
    print(f'GPA: {student_check.gpa}')
...
def problem_six(request):
  ...
  student_update = Student.objects.get(id=11)
  Student.objects.filter('gpa').update(gpa=3.5)
  
  student_check = Student.objects.filter(id=11)
